...
class RETAIN_EX(nn.Module):
    def __init__(self, input_size, hidden_size, num_classes, device=torch.device('cude:0'), bidirectional=True, time_ver=1):
        ...
        emb1 = nn.Embedding(input_size+1, hidden_size, padding_idx=input_size)
        self.emb1 = emb1
        emb2 = nn.Embedding(input_size+1, hidden_size, padding_idx=input_size)
        self.emb2 = emb2

        hidden_input = hidden_size
        if self.time_ver==1:
            self.input_size += 3
            hidden_input = hidden_size+3
        self.RNN1 = nn.LSTM(hidden_input,hidden_size, 1, batch_first=True, bidirectional=self.bidirectional)
        self.RNN2 = nn.LSTM(hidden_input,hidden_size, 1, batch_first=True, bidirectional=self.bidirectional)
    ...
    def forward(self, inputs_list):
        ...
        embedded = (self.emb1(inputs)*mask.unsqueeze(-1)).sum(dim=2)
        embedded2 = (self.emb2(inputs)*mask.unsqueeze(-1)).sum(dim=2)
        if self.release:
            self.embedded = embedded

        # get alpha coefficients
        if self.time_ver==1:
            dates = torch.stack([dates,1/(dates+1),1/torch.log(np.e+dates)],2) # [b x seq x 3]
    ...
    def list_to_tensor(self,inputs): # deals with input preprocessing
        # ver 2
        input_tensor = torch.zeros(len(inputs),len(inputs[0]), 1400, device=self.device, requires_grad=True)
